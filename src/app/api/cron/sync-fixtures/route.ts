...
export async function POST(request: NextRequest) {
  ...
  return executeCronJob('sync-fixtures', async (supabase) => {
    if (!getEnvironmentPolicy().externalFixtureSyncEnabled) {
      throw new CronExecutionError(
        'External fixture synchronization is disabled in this environment.',
        409
      );
    }
    ...
    return {
      ...
      summary: {
        ...
        fixturesUpserted: results.reduce((total, result) => total + result.upserted, 0),
