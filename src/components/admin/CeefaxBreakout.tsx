...
function freshBall(paddleX: number): Ball {
  return { x: paddleX, y: PADDLE_Y - 18, vx: 245, vy: -315, stuck: true, stuckOffset: 0 };
}

function createGame(): Game {
  return {
    balls: [freshBall(WIDTH / 2)], bricks: createBricks(1), drops: [], shots: [],
    paddleX: WIDTH / 2, paddleWidth: 150, keys: { left: false, right: false },
    score: 0, lives: 3, level: 1, catchUntil: 0, laserUntil: 0, wideUntil: 0,
    lastShotAt: 0,
  };
}
...
function drawGame(ctx: CanvasRenderingContext2D, game: Game, now: number) {
  ...
  ctx.fillRect(Math.round(game.paddleX - game.paddleWidth / 2) + 6, PADDLE_Y + 4, Math.round(game.paddleWidth) - 12, 4);
...
function hitsBrick(x: number, y: number, radius: number, brick: Brick) {
  return x + radius >= brick.x && x - radius <= brick.x + brick.width &&
...
export function CeefaxBreakout({
  ...
  const applyPower = useCallback((type: PowerType, now: number) => {
    const game = gameRef.current;
    if (type === 'WIDE') { game.wideUntil = now + 15000; game.paddleWidth = 230; }
    if (type === 'CATCH') game.catchUntil = now + 12000;
    if (type === 'LASER') game.laserUntil = now + 12000;
    if (type === 'SLOW') game.balls.forEach((ball) => { ball.vx *= 0.72; ball.vy *= 0.72; });
    if (type === 'MULTI') {
      const source = game.balls[0];
      if (source) {
        game.balls.push(
          { ...source, vx: -Math.abs(source.vx || 250), vy: -Math.abs(source.vy || 300), stuck: false },
          { ...source, vx: Math.abs(source.vx || 250) * 0.7, vy: -Math.abs(source.vy || 300), stuck: false },
        );
      }
    }
  ...
  const fireAction = useCallback(() => {
    const game = gameRef.current;
    if (screenRef.current === 'splash' || screenRef.current === 'gameover' || screenRef.current === 'won') return;
    if (screenRef.current === 'paused') { changeScreen('playing'); return; }
    const stuck = game.balls.filter((ball) => ball.stuck);
    if (stuck.length) {
      stuck.forEach((ball, index) => {
        ball.stuck = false;
        ball.vx = (index % 2 ? -1 : 1) * 245;
        ball.vy = -315;
      });
      return;
    }
  ...
  useEffect(() => {
    ...
    const nextLevel = () => {
      const game = gameRef.current;
      if (game.level >= 5) { changeScreen('won'); publishHud(); return; }
      game.level += 1;
    ...
    const update = (dt: number, now: number) => {
      const game = gameRef.current;
      if (now > game.wideUntil) game.paddleWidth = 150;
      const direction = Number(game.keys.right) - Number(game.keys.left);
      game.paddleX += direction * 520 * dt;
      game.paddleX = Math.max(game.paddleWidth / 2 + 18, Math.min(WIDTH - game.paddleWidth / 2 - 18, game.paddleX));
      for (const ball of game.balls) {
        if (ball.stuck) { ball.x = game.paddleX + ball.stuckOffset; ball.y = PADDLE_Y - 15; continue; }
        ball.x += ball.vx * dt;
        ball.y += ball.vy * dt;
        if (ball.x < 24) { ball.x = 24; ball.vx = Math.abs(ball.vx); }
        if (ball.x > WIDTH - 24) { ball.x = WIDTH - 24; ball.vx = -Math.abs(ball.vx); }
        if (ball.y < 24) { ball.y = 24; ball.vy = Math.abs(ball.vy); }
        if (ball.vy > 0 && ball.y + 11 >= PADDLE_Y && ball.y < PADDLE_Y + 18 && Math.abs(ball.x - game.paddleX) <= game.paddleWidth / 2 + 8) {
          ball.y = PADDLE_Y - 12;
          ...
          if (now < game.catchUntil) { ball.stuck = true; ball.stuckOffset = ball.x - game.paddleX; }
        }
        for (const brick of game.bricks) {
      ...
      if (!game.balls.length) {
        game.lives -= 1;
        publishHud();
        if (game.lives <= 0) changeScreen('gameover'); else game.balls = [freshBall(game.paddleX)];
      }
      for (const drop of game.drops) drop.y += 120 * dt;
      const caught = game.drops.filter((drop) => drop.y + 12 >= PADDLE_Y && drop.y < PADDLE_Y + 20 && Math.abs(drop.x - game.paddleX) <= game.paddleWidth / 2 + 20);
    ...
    <div className={styles.backdrop} role="dialog" aria-modal="true" aria-label="Football Breakout">
      <section className={styles.shell}>
        <header className={styles.masthead}>
          ...
          <button className={styles.close} type="button" onClick={onClose} aria-label="Close game">×</button>
        </header>
        <div className={styles.hud} aria-live="polite">
          <span>SCORE <strong>{String(hud.score).padStart(6, '0')}</strong></span>
          <span>PLAYER <strong>{player}</strong></span>
          <span>BALLS <strong>{hud.lives}</strong></span>
          <span>ROUND <strong>{hud.level}/5</strong></span>
