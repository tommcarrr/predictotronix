...
## Corpus Check
- 130 files · ~50,385 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 653 nodes · 1107 edges · 48 communities (38 shown, 10 thin omitted)
- Extraction: 100% EXTRACTED · 0% INFERRED · 0% AMBIGUOUS · INFERRED: 4 edges (avg confidence: 0.92)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `9e79a8fe`
...
1. `createServiceClient()` - 58 edges
...
Cohesion: 0.09
Nodes (46): correctResult(), assignLeagueAdmin(), createLeague(), regenerateInviteCode(), toggleInviteActive(), dynamic, LeaguesAdminPage(), metadata (+38 more)
...
- `getSeasonNow()` (2× useful, score=1.939498222)
- `isKickoffLocked()` (2× useful, score=1.939498222)

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `createServiceClient()` connect `createServiceClient` to `server.ts`, `environment.ts`, `fixtures/actions.ts`, `getAdminContext`, `season/route.ts`?**
  _High betweenness centrality (0.074) - this node is a cross-community bridge._
- **Why does `ApiFixture` connect `ApiFixture` to `fixtures/actions.ts`?**
  _High betweenness centrality (0.019) - this node is a cross-community bridge._
- **Why does `isSuperAdmin()` connect `createServiceClient` to `server.ts`, `fixtures/actions.ts`, `getAdminContext`, `season/route.ts`?**
